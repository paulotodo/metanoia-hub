 ...
 *
 * NOTE: the extension is created lazily via a factory function rather than at
 * module load. Calling `Prisma.defineExtension` at top level would run on import,
 * which breaks any test that partially mocks `@prisma/client` (the mock would
 * need to re-export `Prisma`). Deferring it to PrismaService construction keeps
...
export function createPrismaTracingExtension() {
  return Prisma.defineExtension({
    ...
    query: {
      async $allOperations({
        ...
        // At $allOperations level we detect by operation name 'queryRaw' with no
        // model — the keep-alive caller uses $queryRaw`SELECT 1`. Checking the
        // operation name avoids an unsafe cast on args.
