...
export const trailStructureHandlers = [
  ...
  http.get('*/api/v1/trails/:trailId/modules/:moduleId/lessons/:lessonId', ({ params }) => {
    const allLessons = [...mockLessonsModuleA, ...mockLessonsModuleB];
    const lesson = allLessons.find((l) => l.id === params.lessonId);
    if (lesson) {
      return HttpResponse.json({ data: lesson });
    }
    return HttpResponse.json(
      { statusCode: 404, error: 'Not Found', message: 'Aula não encontrada' },
      { status: 404 },
    );
  }),
