...
- Q: Qual o formato de serialização do trecho destacado (highlight) no campo `snippet` da resposta JSON? → A: Tags HTML `<b>termo</b>` geradas pelo `ts_headline` do PostgreSQL (StartSel='<b>', StopSel='</b>'). O FE renderiza com `dangerouslySetInnerHTML` em contexto controlado (campo de busca, sem input de usuário no conteúdo). **[SUPERADA por dec-014 — gate owasp-security HIGH:** a premissa "sem input de usuário no conteúdo" é incorreta (`name`/`tags` são input de autor). Mitigação final = secure by construction: `ts_headline` com sentinelas NÃO-HTML (`\x02`/`\x03`), payload SEM HTML, FE monta `<b>{trecho}</b>` em JSX (React escapa); `dangerouslySetInnerHTML` e DOMPurify PROIBIDOS. Ver research D4.1 / contracts/search-api.md.]**
...
- **FR-005**: Os resultados de busca DEVEM incluir: nome da aula, nome do módulo ao qual pertence, nome da trilha ao qual pertence, tipo de conteúdo, e trecho do texto com o termo encontrado destacado. **Serialização (dec-014, atualiza a clarification original):** o `snippet` é **texto plano** com os matches delimitados por sentinelas NÃO-HTML (`ts_headline` com `StartSel=\x02, StopSel=\x03`); o payload NÃO contém HTML. O FE monta o destaque `<b>{trecho}</b>` em JSX (React escapa o conteúdo do autor) — `dangerouslySetInnerHTML` é PROIBIDO. Ver research D4.1 / contracts/search-api.md.
