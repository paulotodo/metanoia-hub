...
| 2026-05-09 | Code review follow-up: trust proxy em `main.ts`; eviction do rate-limiter no hot path; sanitização de control chars + re-tag `claimedTenantId/claimedUserId` no controller público; whitelist de `error` names no `AllExceptionsFilter`; retry policy passa a tentar 408/425/429; mapping `PlanLimitReached → plan.limitReached` corrige interpolação `{current}/{limit} {resource}`; placeholder `{max}` → `{limit}` em `pt-BR.json` alinhado com payload do `PlanLimitsGuard`; fallback genérico se interpolação ficar incompleta; URL do beacon via `new URL`; useRef guard contra StrictMode double-report; Tailwind v4 token (`text-text-muted`) substitui `[var(--…)]`; Zod rejeita CR/LF e `statusCode < 400`. |
